...
import { useUpdateAssignmentMutation } from "../../services/assignmentApiSlice";
  ...
  questions: Yup.array()
    .of(
      Yup.object().shape({
        ...
        answer: Yup.string().required("Correct answer required"), // ✅ ADDED
...
export default function EditAssignmentModal({
  ...
  const getInitialValues = (assignmentData) => {
    console.log("Assignment data:", assignmentData);

    return {
      title:
        typeof assignmentData?.title === "string" ? assignmentData.title : "",
      description:
        typeof assignmentData?.description === "string"
      ...
      questions: Array.isArray(assignmentData?.questions)
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      alert(err?.data?.message || "Error updating assignment");
    } finally {
      setSubmitting(false);
    }
  };
              ...
              <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-card p-6 text-left align-middle shadow-xl transition-all">
                <div className="mb-4 border-b border-gray-200 pb-3">
                  ...
                  <p className="mt-1 text-sm text-textSecondary">
                    {assignment?.title && typeof assignment.title === "string"
                      ? `"${assignment.title}"`
                      : "Update assignment details and questions below."}
                  </p>
                </div>
                        ...
                        <div className="flex items-center justify-between mb-2">
                          <div>
                            ...
                            <p className="text-xs text-textSecondary">
                              Edit questions with correct answers for
                              auto-grading.
                            </p>
                          </div>
                          <button
